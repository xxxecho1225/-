...
url = 'https://zhaosheng.hdu.edu.cn/list.php?cid=28'
...
def get_provinces(driver):
    ...
    WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'Licon')))
    province_elements = driver.find_elements(By.XPATH, '//*[@id="Licon"]/div[2]/div[1]/form/table[1]/tbody/tr/td/input')
    province_len = len(province_elements)
    for province_id in range(province_len):
        province_elements = driver.find_elements(By.XPATH, '//*[@id="Licon"]/div[2]/div[1]/form/table[1]/tbody/tr/td/input')
        province_name = province_elements[province_id].get_attribute("value")
        print('省份：', province_name)
        province_elements[province_id].click()


        WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'Licon')))
        year_elements = driver.find_elements(By.XPATH, '//*[@id="Licon"]/div[2]/div[1]/form/table[2]/tbody/tr/td/input')
        year_len = len(year_elements)
        for year_id in range(year_len):
            year_elements = driver.find_elements(By.XPATH, '//*[@id="Licon"]/div[2]/div[1]/form/table[2]/tbody/tr/td/input')
            year_name = year_elements[year_id].get_attribute("value")
            print('年份：', year_name)
...
def save_into_csv(data, year_name, province_name):
    ...
    # 保存文件
    filename = f"{year_name}-{province_name}-杭州电子科技大学.csv"
    os.makedirs('学校信息\杭州电子科技大学', exist_ok=True)
    wb.save(os.path.join('学校信息\杭州电子科技大学', filename))


    
def clickbutton(driver):
    button = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[@id="Licon"]/div[2]/div[1]/form/input[1]'))) 
    # 点击按钮
    button.click()
...
def get_table_data(driver):
    try:
        WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.ID, 'result')))
        data_wrapper = driver.find_element(By.ID, 'result')
    except NoSuchElementException:
        print("未找到数据包装器元素，跳过当前省份处理")
        return []
    rows = data_wrapper.find_elements(By.XPATH, '//*[@id="result"]')
    data = []
    for row in rows:
        try:
