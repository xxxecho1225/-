...
url = 'https://zhinengdayi.com/page/detail/NLGYFE/83/220'
...
def get_provinces(driver):
    ...
    WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'cityName_1')))
    province_elements = driver.find_elements(By.XPATH, '//*[@id="cityName_1"]/option')
    ...
    for province_id in range(province_len):
        ...
        WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'year')))
        year_elements = driver.find_elements(By.XPATH, '//*[@id="year"]/option')
        ...
        for year_id in range(year_len):
            ...
            WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'scienceClass_1')))
            kelei_elements = driver.find_elements(By.XPATH, '//*[@id="scienceClass_1"]/option')
            ...
            for kelei_id in range(kelei_elements_len):
                ...
                data_all[f'{kelei_name}'] = copy.deepcopy(data)
...
def save_into_csv(data, year_name, province_name, driver):
    ...
    for subject_type_key, rows in data.items():
        # 替换斜杠为下划线
        clean_kelei_name = subject_type_key.replace("/", "_")
        ws_title = f"{clean_kelei_name}"
        # 移除不合法字符
        ws_title = "".join(c for c in ws_title if c.isalnum() or c in "_-")
    ...
    filename = f"{year_name}-{province_name}-对外经济贸易大学.csv"
    os.makedirs('学校信息\对外经济贸易大学', exist_ok=True)
    wb.save(os.path.join('学校信息\对外经济贸易大学', filename))

def get_table_header(driver):
    WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.ID, 'majorCompareList')))
    header = driver.find_element(By.ID, 'majorCompareList')
    column_headers = header.find_elements(By.XPATH, '//*[@id="majorCompareList"]/tbody/tr/th[position() != 2]')
...
def get_table_data(driver):
    try:
        WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.ID, 'majorCompareList')))
        data_wrapper = driver.find_element(By.ID, 'majorCompareList')
    except NoSuchElementException:
        print("未找到数据包装器元素，跳过当前省份处理")
        return []
    rows = data_wrapper.find_elements(By.XPATH, '//*[@id="majorCompareList"]/tbody/tr[position() > 1]')
    data = []
    for row in rows:
        try:
            ...
            rows = data_wrapper.find_elements(By.XPATH, '///*[@id="majorCompareList"]/tbody/tr')
            continue
    print("表格数据：", data)
    return data
