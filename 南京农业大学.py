...
url = 'https://zsxx.njau.edu.cn/lnlqfs.jsp?urltype=tree.TreeTempUrl&wbtreeid=1024'
...
def get_provinces(driver):
    ...
    year_elements = driver.find_elements(By.XPATH, '//*[@id="nf"]/option[position() > 1]')
    ...
    for year_id in range(year_len):
        ...
        WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'sf')))
        province_elements = driver.find_elements(By.XPATH, '//*[@id="sf"]/option[position() > 1]')
...
def clickbutton(driver):
    next_button = driver.find_element(By.XPATH, '//*[@id="imptdata_inquire_u10"]/div/div[4]/input')
    next_button.click()
...
def save_into_csv(data, year_name, province_name):
    ...
    filename = f"{year_name}-{province_name}-南京农业大学.csv"
    os.makedirs('学校信息\南京农业大学', exist_ok=True)
    wb.save(os.path.join('学校信息\南京农业大学', filename))


def get_table_data(driver):
    try:
        WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.CLASS_NAME, 'cxjg')))
        data_wrapper = driver.find_element(By.CLASS_NAME, 'cxjg')
    except NoSuchElementException:
        print("未找到数据包装器元素，跳过当前省份处理")
        return []
    rows = data_wrapper.find_elements(By.XPATH, '/html/body/div[5]/div[2]/div[4]/table/tbody/tr')
    data = []
    for row in rows:
        try:
