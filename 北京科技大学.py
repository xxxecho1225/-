...
url = 'https://zhaosheng.ustb.edu.cn/zkxx/lnfs/index.htm'
...
def get_provinces(driver):
    ...
    WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'provinceSelectListID')))
    province_elements = driver.find_elements(By.XPATH, '//*[@id="provinceSelectListID"]/a')
    ...
    for province_id in range(province_len):
        ...
        WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'yearSelectListID')))
        year_elements = driver.find_elements(By.XPATH, '//*[@id="yearSelectListID"]/a')
        ...
        for year_id in range(year_len):
            ...
            WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'keleiSelectListID')))
            kelei_elements = driver.find_elements(By.XPATH, '//*[@id="keleiSelectListID"]/a')
...
def save_into_csv(data,year_name,province_name, driver):
    ...
    filename = f"{year_name}-{province_name}-北京科技大学.csv"
    os.makedirs('学校信息\北京科技大学', exist_ok=True)
    wb.save(os.path.join('学校信息\北京科技大学', filename))

def get_table_header(driver):
    WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'subTableBox')))
    header = driver.find_element(By.CLASS_NAME, 'subTableBox')
    column_headers = header.find_elements(By.XPATH, '/html/body/article/div/section/div/table/thead/tr/th')
...
def get_table_data(driver):
    try:
        WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.ID, 'lnfsTbodyID')))
        data_wrapper = driver.find_element(By.ID, 'lnfsTbodyID')
    except NoSuchElementException:
        print("未找到数据包装器元素，跳过当前省份处理")
        return []
    rows = data_wrapper.find_elements(By.XPATH, '//*[@id="lnfsTbodyID"]/tr')
    data = []
    for row in rows:
        try:
