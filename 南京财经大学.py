...
url = 'http://bkzs.nufe.edu.cn/ldcx.jsp?urltype=tree.TreeTempUrl&wbtreeid=1023'
...
def get_provinces(driver):
    ...
    WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.ID, 'nf')))
    year_elements = driver.find_elements(By.XPATH, '//*[@id="nf"]/option[position() != 7]')
    ...
    for year_id in range(year_len):
        ...
        save_into_csv(data, year_name,driver)

def save_into_csv(data, year_name, driver):
    wb = Workbook()
    # 首先删除默认创建的工作表
    ...
    # 保存文件
    filename = f"{year_name}-南京财经大学.csv"
    os.makedirs('学校信息\南京财经大学', exist_ok=True)
    wb.save(os.path.join('学校信息\南京财经大学', filename))


    
def clickbutton(driver):
    button = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[@id="imptdata_inquire_u8"]/input')))
    # 点击按钮
    button.click()

def get_table_header(driver):
    WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'table_box')))
    header = driver.find_element(By.CLASS_NAME, 'table_box')
    column_headers = header.find_elements(By.XPATH, '/html/body/div[4]/div/div[2]/div[2]/div[2]/div/div/div[2]/table/thead/tr/th')
...
def get_table_data(driver):
    try:
        WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.CLASS_NAME, 'table_box')))
        data_wrapper = driver.find_element(By.CLASS_NAME, 'table_box')
    except NoSuchElementException:
        print("未找到数据包装器元素，跳过当前省份处理")
        return []
    rows = data_wrapper.find_elements(By.XPATH, '/html/body/div/div/div/div/div/div/div/div/table/tbody/tr')
    data = []
    for row in rows:
        try:
