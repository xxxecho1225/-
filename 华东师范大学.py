...
def get_provinces(driver):
    ...
    province_elements = driver.find_elements(By.XPATH, '//*[@id="filterPlace"]/dl[1]/dd/a[text() = "广东"]')
        ...
        for year_id in range(year_len):
            ...
            for type_id in range(type_len):
                ...
                for subject_id in range(subject_len):
                    ...
                    WebDriverWait(driver, 200).until(EC.presence_of_element_located((By.ID, 'filterPlace')))
                    zyz_elements = driver.find_elements(By.XPATH, '//*[@id="filterPlace"]/dl[5]/dd/a')
                    zyz_len = len(zyz_elements)
                    if zyz_len == 1:
                    # 如果专业组长度为1，则跳过处理，直接爬取数据并存储
                        data = get_table_data(driver)
                        data_all[f'{type_name}_{subject_name}'] = copy.deepcopy(data)
                    else:
                        # 有专业组时，继续遍历专业组并爬取数据
                        for zyz_id in range(zyz_len):
                            zyz_elements = driver.find_elements(By.XPATH, '//*[@id="filterPlace"]/dl[5]/dd/a')
                            zyz_name = zyz_elements[zyz_id].text
                            print('专业组：', zyz_name)
                            print('专业组长度：', zyz_len)
                            zyz_elements[zyz_id].click()
                            # 爬取数据并存储
                            data = get_table_data(driver)
                            data_all[f'{type_name}_{subject_name}_{zyz_name}'] = copy.deepcopy(data)
                            # 在保存到 CSV 文件之前，确保数据字典中至少包含了一个条目
                    if data_all:
