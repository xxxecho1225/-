...
url = 'https://admission.lcu.edu.cn/static/front/lcu/basic/html_web/lnfs.html'
...
def saveIntoCsv(data, province_name, year_name):
    ...
    for subject_type_key, rows in data.items():
        ...
        for row in rows:
            ws.append(row)
    # 保存工作簿到文件
    filename = f"{year_name}-{province_name}-聊城大学.csv"
    os.makedirs(f'学校信息/聊城大学', exist_ok=True)
    wb.save(os.path.join(f'学校信息/聊城大学', filename))
