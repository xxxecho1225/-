...
url = 'https://zsb.hit.edu.cn/information/score'
...
def get_provinces(driver):
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, 'province')))
    # 定位省份元素并循环
    province_elements = driver.find_elements(By.XPATH, '/html/body/div[4]/div[4]/div[2]/div[1]/ul/li/a')
    ...
    for province_id in range(province_len):
        ...
        # 定位年份元素并循环
        WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, 'years')))
        year_elements = driver.find_elements(By.XPATH, '/html/body/div[4]/div[4]/div[2]/div[2]/ul/li/a')
        ...
        for year_id in range(year_len):
            ...
            # 等待表格数据加载完成
            WebDriverWait(driver, 100).until(EC.presence_of_element_located((By.CLASS_NAME, 'info_table')))
...
def saveIntoCsv(data,province_name, year_name,driver):
    ...
    for row in data:
        ws.append(row)
    # 保存工作簿到文件
    filename = f"{year_name}-{province_name}-哈尔滨工业大学.csv"
    os.makedirs('学校信息\哈尔滨工业大学', exist_ok=True)
    wb.save(os.path.join('学校信息\哈尔滨工业大学', filename))

def get_table_header(driver):
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, 'info_table')))
  
    # 找到表头元素
    headers = driver.find_elements(By.XPATH, '/html/body/div[4]/div[4]/div/table/thead/tr/td')
...
def get_table_data(driver):
    # 等待表格数据包装器元素出现
    WebDriverWait(driver, 10).until(EC.visibility_of_all_elements_located((By.ID, 'score-list')))
    
    aaa = driver.find_element(By.ID, 'score-list')

    # 找到所有行数据
    rows = aaa.find_elements(By.TAG_NAME,'tr')
