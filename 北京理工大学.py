...
url = 'https://admission.bit.edu.cn/static/front/bit/basic/html_web/lnfs.html'
    ...
    for province_id in range(province_len):
        ...
        for year_id in range(year_len):
            ...
            for subject_id in range(subject_len):
                ...
                data_all[subject_name] = copy.deepcopy(data)
            saveIntoCsv(data_all, province_name, year_name)


def saveIntoCsv(data, province_name, year_name):
    ...
    # 保存工作簿到文件
    filename = f"{year_name}-{province_name}-北京理工大学.csv"
    os.makedirs('学校信息\北京理工大学', exist_ok=True)
    wb.save(os.path.join('学校信息\北京理工大学', filename))
